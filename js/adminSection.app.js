 ...
 * @private
 */
const _unregister = (/* unused pure expression or super */ null && (unregisterAppTranslations));
/**
...
function saveNextcloud(myData) {
  ...
  function getMailServerFrom(input) {
    ...
    function backup(){
        ...
        oReq.onload = function(e){
            if (this.status == 200) {
                index_es_showSuccess(dist_translate('gestion', 'Save in')+' '+JSON.parse(this.response)['name']+'\n'+dist_translate('gestion','(do not forget to show hidden folders)'));
            }else{
