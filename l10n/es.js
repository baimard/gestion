OC.L10N.register(
    "gestion",
    {
    "Name" : "Nombre",
    "First name" : "Apellidos",
    "Limited company" : "Empresa Limitada",
    "Company" : "Empresa",
    "Phone number" : "Número de teléfono",
    "Email" : "Correo electrónico",
    "Address" : "Dirección",
    "Quote number" : "Número de presupuesto",
    "New" : "Nuevo",
    "Comment" : "Comentario",
    "Text free" : "Texto libre",
    "Invoice number" : "Número de factura",
    "Means of payment" : "Métodos de pago",
    "INVOICE" : "FACTURA",
    "Reference" : "Referencia",
    "Designation" : "Designación",
    "Your company name" : "Nombre de tu empresa",
    "Your company contact surname" : "Apellido del contacto en tu empresa",
    "Your company contact name" : "Nombre del contacto en tu empresa",
    "Your company email" : "Su correo de empresa",
    "Your company phone" : "Teléfono de tu empresa",
    "Your company address" : "Dirección de tu empresa",
    "All Legal mentions, disclaimer or everything, you want place in the footer." : "Todas las menciones legales, descargo de responsabilidad o cualquier otra cosa que quieras colocar en el pie de página.",
    "EUR" : "EUR",
    "Gestion" : "Gestion",
    "Simplified invoice management application" : "Una aplicación de gestión de facturas simplificada",
    "# Gestion Nextcloud application\n\n        This application allows the management of invoicing in a very simple way for Micro-Entreprise.\n\n        Functionality:\n\n        * Add customer\n        * Generate a quote\n        * Generate an invoice\n        * Generate product lines to include in quotes\n        * You can generate PDFs directly from the app and save them to your Nextcloud\n\n        Presentation video in user documentation\n\n        If you need to adapt this application to your country laws, please open issue.\n\n        [Love my work ? Buy me a coffee !](https://www.buymeacoffee.com/benjaminaimard)" : "# Gestión: aplicación Nextcloud\n\n        Esta aplicación te permite gestionar tus facturas de forma sencilla para micro-empresas.\n\n        Funcionalidad:\n\n        * Añadir un cliente\n        * Generar un presupuesto\n        * Generar una factura\n        * Crear líneas de productos para incluir en presupuestos\n        * Puedes generar PDFs directamente desde la aplicación y guardarlas en tu Nextcloud\n\n        Vídeo de presentación en la documentación de usuario\n\n        Si necesitas adaptar esta aplicación debido a la legislación de tu país, por favor abre un ticket.\n\n        [¿Te encanta mi trabajo? ¡Cómprame un café!](https://www.buymeacoffee.com/benjaminaimard)",
    "Choose work folder" : "Elige un directorio de trabajo",
    "Please create a new product" : "Por favor cree un nuevo producto",
    "Email sent" : "Correo enviado",
    "Is your global mail server configured in Nextcloud?" : "¿Está configurado su servidor de correo global en Nextcloud?",
    "Creation in progress …" : "Creación en progreso",
    "Welcome to GESTION 2.0" : "Bienvenido a GESTIÓN 2.0",
    "To start with this application you need to configure your company information, follow this link" : "Para empezar con esta aplicación debes configurar la información de tu empresa, sigue este enlace",
    "My company" : "Mi empresa",
    "If you like my work you can" : "Si te gusta mi trabajo puedes",
    "buy me a coffee" : "comprarme un café",
    "If you need documentation, follow this link" : "Si necesitas documentación, sigue este enlace",
    "Documentation" : "Documentación",
    "Others questions?" : "¿Más preguntas?",
    "Contact" : "Contacto",
    "Leave me a comment, but only if you like this application :)" : "Escríbeme un comentario, pero solo si te gusta esta aplicación :)",
    "Nextcloud apps" : "Aplicaciones Nextcloud",
    "Want to talk with the community?" : "¿Quieres hablar con la comunidad?",
    "Git discussion" : "Discusión Git",
    "Changelog" : "Registro de cambios",
    "Send an email" : "Enviar un correo",
    "Your invoice/quote" : "Su factura/presupuesto",
    "Dear," : "Estimado,",
    "Attached to this email you will find a new document." : "Adjunto a este correo encontrará un nuevo documento",
    "Best regards," : "Un cordial saludo,",
    "Send" : "Enviar",
    "Help configure" : "Ayuda a configurar",
    "Company name" : "Nombre de la empresa",
    "Automatic generated invoice number" : "Número de factura generado automáticamente",
    "Global default Currency" : "Divisa global por defecto",
    "Legal disclaimer/mentions" : "Descargo de responsabilidad legal/menciones",
    "Legal disclaimer/mentions you need in your footer - before company information" : "Descargo de responsabilidad/menciones que necesitas poner en tu pie de página - antes de la información de la empresa",
    "Configuration" : "Configuración",
    "Click to edit" : "Haz clic para editar",
    "This is the name of the company that will appear in the footer of your quote and invoice.  For example, you can set : \"Company : Cybercorp\" or just \"Cybercorp\"" : "Este es el nombre de empresa que aparecerá en el pie de página del presupuesto y factura. Por ejemplo, puedes escribir: \"Empresa: Cybercorp\" o simplemente \"Cybercorp\"",
    "Loading …" : "Cargando …",
    "First name appear in the quote/invoice header as a contact" : "Nombre de pila que aparecerá en la cabecera del presupuesto/factura como contacto",
    "Last name appear in the quote/invoice header as a contact" : "Apellido(s) que aparecerá en la cabecera del presupuesto/factura como contacto",
    "Company address appear in the quote/invoice header" : "Dirección de empresa, aparece en la cabecera del presupuesto/factura",
    "This is a first line in the footer of your Quote/Invoice with all legals informations you need" : "Esta es la primera línea del pie de página de tu presupuesto/factura con toda la información legal que sea necesaria",
    "This is a second line in the footer of your Quote/Invoice with all legals informations you need" : "Esta es la segunda línea del pie de página de tu presupuesto/factura con toda la información legal que sea necesaria",
    "Phone number appear in the quote/invoice header" : "Número de teléfono, aparece en la cabecera del presupuesto/factura",
    "Email appear in the quote/invoice header" : "Correo electrónico, aparece en la cabecera del presupuesto/factura",
    "Global Default VAT apply to your Quote/Invoice (globally), to change it, please just insert VAT amount without the percent sign" : "IVA global por defecto a aplicar a tu presupuesto/factura (de forma global), para cambiarlo, inserta el IVA sin el símbolo de porcentaje",
    "Quote" : "Presupuesto",
    "Add quote" : "Añadir presupuesto",
    "Quote date" : "Fecha del presupuesto",
    "Customer quote" : "Presupuesto de cliente",
    "Version" : "Versión",
    "Status" : "Estado",
    "Actions" : "Acciones",
    "Quote #" : "Presupuesto #",
    "FROM" : "DESDE",
    "TO" : "A",
    "Offer valid for 1 month from" : "Oferte válida por 1 mes desde",
    "Quantity" : "Cantidad",
    "Unit price without VAT" : "Precio sin IVA",
    "Total without VAT" : "Total sin IVA",
    "Add product" : "Añadir producto",
    "Save in Nextcloud" : "Guardar en Nextcloud",
    "Send by email" : "Enviar por correo",
    "VAT Rate" : "Tipo de IVA",
    "Total VAT" : "Total IVA",
    "Total Price" : "Precio Total",
    "Please set in global configuration" : "Por favor, establezcalo en la configuración global",
    "Invoice" : "Factura",
    "Add invoice" : "Añadir factura",
    "Date of service" : "Fecha del servicio",
    "Payment date" : "Fecha de pago",
    "Associated quote" : "Presupuesto asociado",
    "Invoice #" : "Factura #",
    "Settlement date" : "Fecha de liquidación",
    "Customer" : "Cliente",
    "Add customer" : "Añadir cliente",
    "Legal information" : "Información legal",
    "Product" : "Producto",
    "Statistics" : "Estadísticas",
    "Monthly turnover without VAT" : "Facturación mensual sin IVA",
    "Year" : "Año",
    "January" : "Enero",
    "February" : "Febrero",
    "March" : "Marzo",
    "April" : "Abril",
    "May" : "Mayo",
    "June" : "Junio",
    "July" : "Julio",
    "August" : "Agosto",
    "September" : "Septiembre",
    "October" : "Octubre",
    "November" : "Noviembre",
    "December" : "Diciembre",
    "Total" : "Total",
    "Manage" : "Gestionar",
    "Information" : "Información",
    "About" : "Acerca de",
    "Save folder" : "Directorio de guardado",
    "Please choose a folder" : "Por favor, elige una carpeta"
},
"nplurals=2; plural=(n != 1);");
