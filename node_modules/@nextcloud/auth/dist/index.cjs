...
function setRequestToken(token) {
  ...
  eventBus.emit("csrf-token-update", { token, _internal: true });
}
async function fetchRequestToken() {
  const url = router.generateUrl("/csrftoken");
...
function onRequestTokenUpdate(observer) {
  ...
  eventBus.subscribe("csrf-token-update", wrapper);
  return () => eventBus.unsubscribe("csrf-token-update", wrapper);
}
function _subscribeToTokenUpdates() {
  eventBus.subscribe("csrf-token-update", ({ token, _internal }) => {
    if (!_internal) {
      setRequestToken(token);
    }
  });
...
exports.setRequestToken = setRequestToken;
